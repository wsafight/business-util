...
function _distance(str1: string, str2: string) {
  ...
  const distance: number = levenshtein(str1, str2);
    ...
    for (let j = 0; j <= str1.length; j++) {
      if (i && j) {
        if (str1.charAt(j - 1) === str2.charAt(i - 1)) {
          ...
          value = Math.min(current[j], current[j - 1], prev as number) + 1;
        }
      } else {
        value = i + j;
      }
...
function sort_descending(a: number[], b: number[]) {
  return b[0] - a[0];
}
